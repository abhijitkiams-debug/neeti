...
type Data = { vendorUser: { id: string; name: string; mobile: string; vendorOrgId: string; vendorOrgName: string }; items: Item[] };

export default function VendorUserSignoffPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = usePromise(params);
  const [data, setData] = useState<Data | null>(null);

  useEffect(() => {
    fetch(`/api/reports/vendor-user/${id}`)
      .then((r) => r.json())
      .then(setData);
  }, [id]);
  ...
  return (
    <div className="max-w-3xl">
      <Link href={`/admin/vendors/${data.vendorUser.vendorOrgId}`} className="text-xs text-indigo-600 hover:underline">
        ← {data.vendorUser.vendorOrgName}
      </Link>
      <div className="mt-2 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">{data.vendorUser.name}</h1>
          <p className="mt-1 text-sm text-slate-500">
            {data.vendorUser.mobile} · {data.vendorUser.vendorOrgName}
          </p>
        </div>
          ...
          {data.items.length === 0 && (
            <tr>
              <td colSpan={5} className="px-4 py-6 text-center text-slate-500">
                No policies currently apply to this vendor user.
              </td>
            </tr>
          )}
        </tbody>
